...
class LicenseBoxAPI
{
    ...
    {
        $this->product_id = '50C9EA9D';
        $this->api_url = 'https://licencefly.com/';
        $this->api_key = '4FD23138912D5C2299FA';
    ...
    public function verify_license($time_based_check = false, $license = false, $client = false)
    {
        ...
        if ($time_based_check && $this->verification_period > 0) {
            ...
            $today = date('d-m-Y');
            if (empty($_SESSION["2ddae48d928b219"])) {
                $_SESSION["2ddae48d928b219"] = '00-00-0000';
            }
            if ($type == 1) {
                $type_text = '1 day';
            } elseif ($type == 3) {
            ...
            if (strtotime($today) >= strtotime($_SESSION["2ddae48d928b219"])) {
                $get_data = $this->call_api(
                    'POST',
